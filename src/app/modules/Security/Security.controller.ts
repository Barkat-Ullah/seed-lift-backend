...
const createPageContent = catchAsync(async (req: Request, res: Response) => {
  const { type, title, description } = req.body;
  const page = await PageContentServices.createPageContent({
    type,
    title,
    description,
  });
...
const getAllPages = catchAsync(async (req: Request, res: Response) => {
  const page = await PageContentServices.getAllContent();
  sendResponse(res, {
    statusCode: httpStatus.OK,
...
const updatePageContent = catchAsync(async (req: Request, res: Response) => {
  ...
  const page = await PageContentServices.updatePageContent(
    type,
    title,
    description,
  );
